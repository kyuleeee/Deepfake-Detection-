...
class GenConViT(nn.Module):
        ...
        if self.net == 'ed':
            try:
                ...
                self.checkpoint_ed = torch.load(
                    f'model/GenConViT/{ed}.pth', map_location=torch.device('cpu'))

                if 'state_dict' in self.checkpoint_ed:
                    self.model_ed.load_state_dict(
                        self.checkpoint_ed['state_dict'])
                else:
                ...
                raise Exception(
                    f"Error: model/GenConViT/{ed}.pth file not found.")
        elif self.net == 'vae':
            try:
                self.model_vae = GenConViTVAE(config)
                self.checkpoint_vae = torch.load(
                    f'model/GenConViT/{vae}.pth', map_location=torch.device('cpu'))

                if 'state_dict' in self.checkpoint_vae:
                    self.model_vae.load_state_dict(
                        self.checkpoint_vae['state_dict'])
                else:
                ...
                raise Exception(
                    f"Error: model/GenConViT/{vae}.pth file not found.")
        else:
            try:
                self.model_ed = GenConViTED(config)
                self.model_vae = GenConViTVAE(config)
    ...
    def extract_features(self, x):
        if self.net == 'ed':
            x = self.model_ed.extract_features(x)
        elif self.net == 'vae':
            x = self.model_vae.extract_features(x)
        else:
            x1 = self.model_ed.extract_features(x)
            x2 = self.model_vae.extract_features(x)
            x = torch.cat((x1, x2), dim=1)
        return x
    
    def extract_AE(self, x):
        if self.net == 'ed':
            x = self.model_ed.extract_AE(x)
        elif self.net == 'vae':
            x, _ = self.model_vae(x)
        else:
            x1 = self.model_ed.extract_AE(x)
            x2, _ = self.model_vae(x)
            x = (x1 + x2) / 2
        return x